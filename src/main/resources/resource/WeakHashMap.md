...
```
/**
 * 基于Hash的符号表(继承于Map接口),它的key是弱引用
 * 1.在WeakHashMap中,如果一个key上时间没有被引用的话,那么会被优化的被垃圾回收
 * 2. null值和null的键都是允许的
 * 3.是线程不安全的
 * 4.该Map使用equals方法的作用相当于==操作,因为在WeakHashMap中的键会被清理
 * 5.WeakHashMap的行为依赖于垃圾回收的具体表现,除外它关于读的方法都不是十分精准（因为
 * 元素随时可能被垃圾回收掉）
 * 6.在WeakHashMap中的元素是非直接的作为一个弱引用存在,所以不管在map中后者是在map外
 * 都优化被垃圾回收掉
 * 7.有一点需要注意: 在WeakHashMap中的值是强引用,所以注意不要将值直接强引用到键中，不然会
 *  导致失去了弱引用的价值
 * 8.iterator 参考HashMap中的
 */
