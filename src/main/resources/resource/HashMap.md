...
基于Hash实现的Map，最常用的一个Map

```

/**
 * 1.基于Hash算法的Map，实现了map接口的所有操作，以及这个map的key以及value都是允许
 * 插入null的。
 * 2.这个类除了在线程安全性不同之前，其他的都和HashTable差不多。
 * 3.并不能保证map顺序的一致性（基于hash以及经常rebuild）
 ...
 * 
 * 4.如果hash函数使元素分配的比较均匀的话,那么基本方法get，put的时间是一致的(多次插入和多次
 * 获取同一个元素时间在一定的时间内都将是一样的)
 * 5.在map返回的集合（entrySet,keySet,values的返回值）可能的遍历次数为: capacity + 里面的元素个数
 * ,所以在经常使用遍历操作的场景下不建议容量设置的太大了
 ...
 * important.
 * 
 * 6.在HashMap中有两个参数非常的重要，严重的影响到了它的行为:initial capacity 以及load factor
 * capacity表示的是它的总的容量,initial capacity 表示的是在它初始化的时候的容量;load factor
 * 表示的是在容量占了多少之后可以提前的将下一次的容量给算出来,当实际上的容量已经超过当前的capacity*load factor
 * 那么就会重新的分配容量,并且重新的更具hash分配位置，所以根据这个我们就知道了: capacity 大约是被hash元素的两倍（number of buckets）
 ...
 * entries in the hash table exceeds(超过) the product of the load factor and the
 * current capacity, the hash table is <i>rehashed</i> (that is, internal data
 * structures are rebuilt) so that the hash table has approximately twice the
 * number of buckets.
 *
 * 7.在一般的情况下,默认的load factor为.75，这个值是在空间以及时间的权衡之下设置的
 * 如果空间小了的话,那么查询时间就会很变长,反之亦然(空间以及时间的权衡)
 * 8.但是如果initial capacity大于最大的容量,那么就不会出现重新的hash分配
...
public class HashMap<K,V> extends AbstractMap<K,V>
     ...
     * 最大的容量，这个容量必须是2的倍数，最大为1<<30
     */
    static final int MAXIMUM_CAPACITY = 1 << 30;

    /**
     * The load factor used when none specified in constructor.
     * 如果没有定义的load factor ，那么这个就是默认的值
     */
    static final float DEFAULT_LOAD_FACTOR = 0.75f;

    /**
     * 当同一个hashCode值（hashCode碰撞）的元素多于TREEIFY_THRESHOLD的时候,那么将不会以链表
     * 的形式储存,将会以树形的结构储存。这个值必须大于2,应该最小为8这样方便在树形结构
     * 和链式结构之间转化
     *
    ...
    /**
     * 当同一个hashCode（hashCode碰撞）的元素树形结构少于UNTREEIFY_THRESHOLD的时候
     * 那么将会被重新的打散变为链式
     *
    ...
    /**
     * table在大于MIN_TREEIFY_CAPACITY的时候,才有可能在hashCode碰撞的时候
     * 的时候构造树形结构
     *
     ...
     * 储存了Map中的键值对,在第一次使用的是被初始化,在必要的时候会被被重新的分配
     * table的length都是2的倍数,但是在一种情况下可能不是,那就是刚刚初始化的时候
     */
     ...
     * HashMap的结构改变的次数，利用这个值在Iterator上使用保证快速失败
     */
    ...
    /**
     * 初始化或者使hashMap的容量变为两倍，如果是null的话,那么分配一个再
     * threshold 设置的初始值
